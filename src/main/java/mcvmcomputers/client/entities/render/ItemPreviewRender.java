...
public class ItemPreviewRender extends EntityRenderer<EntityItemPreview> {
	...
	@Override
	public void render(EntityItemPreview entity, float yaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
		...
		MinecraftClient.getInstance().getItemRenderer().renderItem(entity.getPreviewedItemStack(), Mode.NONE, 200, 0, matrices, vertexConsumers, new Random().nextInt());
		matrices.pop();
		matrices.pop();
	}
}
